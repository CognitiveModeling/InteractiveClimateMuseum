...
public class EnvironmentUpdate : MonoBehaviour
{
    ...
    {
        this.sliderValuesQuery = getSliderValues.GetQueryParameters(currURL); //turn URL to one string that complies to the format required by the API

        if (sliderValuesQuery.Count > 0)
        {
            restAPICall.RunModelAction();
            
            // old version (before nov/dec 2023): instead of rest api, we called en_roads.exe:
            //cmdInterface.getTemp2100(sliderValues); //feed sliderValues to API, receive temp(2100) prognosis, apply this value to the environment
        }
        else
        {
