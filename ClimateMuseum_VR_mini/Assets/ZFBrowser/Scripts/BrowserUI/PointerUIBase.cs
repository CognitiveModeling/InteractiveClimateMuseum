...
namespace ZenFulcrum.EmbeddedBrowser
{
    ...
    public abstract class PointerUIBase : MonoBehaviour, IBrowserUI
    {
        ...
        public virtual void Awake()
        {
            ...
            // register event for handling the pointers
        ...
        public virtual void InputUpdate()
        {
            ...
            // clear list and number of pointers at any position
            p_currentDown = p_anyDown = p_currentOver = p_anyOver = -1;
            currentPointers.Clear();

            // call pointer handling
            onHandlePointers();

            // select a pointer and click
            CalculatePointer();
        }
        ...
        public virtual void FeedPointerState(PointerState state)
        {
            ...
            // convert 2D pointers to browser coordinates, see MapPointerToBrowser() in PointerUIMesh
            if (state.is2D) state.position2D = MapPointerToBrowser(state.position2D, state.id);
            // convert 3D pointers to browser coordinates, see MapRayToBrowser() in PointerUIMesh
            else
            {
        ...
        protected virtual void CalculatePointer()
        {
             ...
             */
            // selects one of the current pointers
            ...
            MouseIsOver();
            // calls the Click event action in the browser
            ...
            // no buttons held (or being released), no need to fiddle with the position
            else
            {
